...
// // //       "exp://192.168.1.100:19000", // Expo mobile (adjust IP)
// // //     ],
// // //     methods: ["GET", "POST"],
// // //     credentials: true,
// // //   },
...
// --- CORS / Origins ---
const HOST_IP = process.env.HOST_IP || "10.0.0.192"; // set in .env if you want
...
// // --- CORS / Origins ---
// // set in .env if you want (defaults to your LAN IP)
// const HOST_IP = process.env.HOST_IP || "10.0.0.192";

// // Allowed origins list (web + expo web + expo go scheme bound to your IP)
// // NOTE: Expo native often sends *no* origin; we handle that below with a function.
// const ALLOWED_ORIGINS = [
//   "http://localhost:3000", // web dev
//   "http://localhost:19006", // Expo web
//   `exp://${HOST_IP}:19000`, // Expo Go on / Dynamic IP so you only update one IP
//   // You can keep another example here:
//   // "exp://192.168.1.100:19000",
// ];

// // -------- Express CORS (dev-friendly; allows native/no-origin) --------
// app.use(
//   cors({
//     origin: (origin, cb) => {
//       // Allow requests from localhost, your LAN, or no origin (native apps)
//       if (!origin) return cb(null, true); // Expo Go / native has no origin
//       if (ALLOWED_ORIGINS.includes(origin)) return cb(null, true);

//       // also allow any http://<HOST_IP>:<port> while developing
//       const lanPattern = new RegExp(`^http://${HOST_IP}:\\d+$`);
//       if (lanPattern.test(origin)) return cb(null, true);

//       // DEV: be permissive. For production, tighten this up.
//       return cb(null, true);
//     },
//     credentials: true,
//     methods: ["GET", "POST"],
...
// // -------- Socket.io (matching CORS; dev-friendly) --------
// const io = new Server(server, {
//   cors: {
//     origin: (origin, cb) => {
...
//       const lanPattern = new RegExp(`^http://${HOST_IP}:\\d+$`);
//       if (lanPattern.test(origin)) return cb(null, true);
//       return cb(null, true); // DEV: permissive
//     },
//     credentials: true,
//     methods: ["GET", "POST"],
//   },
//   // Enable polling fallback for problematic networks + keep-alives
//   transports: ["websocket", "polling"],
//   pingInterval: 20000,
//   pingTimeout: 20000,
// });
...
// // ===== In-memory state (demo only; use DB in production) =====
// const chatRooms = new Map();
// const userSessions = new Map();

// // Optional: pre-seed a few rooms so /api/chat/rooms isn’t empty
// if (chatRooms.size === 0) {
//   ["general", "development", "random"].forEach((id) =>
//     chatRooms.set(id, {
//       id,
//       messages: [],
//       participants: new Map(),
...
// // ---- Socket.io events (single connection block; chat + collab) ----
// io.on("connection", (socket) => {
//   console.log(`User connected: ${socket.id}`);
...
//   // Basic ping/pong (from your original) — handy for testing latency
//   socket.on("ping", (data) => {
//     socket.emit("pong", { ...data, serverTimestamp: new Date().toISOString() });
//   });
...
//   // New message (DEDUPED emits; one broadcast + one delivery ack)
//   socket.on("send_message", (data) => {
//     const { roomId, message } = data;
//     const room = chatRooms.get(roomId);
...
//       id: `msg_${Date.now()}_${Math.floor(Math.random() * 7000)}`,
//       roomId, // ✅ include roomId
//       ...message, // may include tempId, userId, userName, text, type
//       timestamp: new Date().toISOString(),
//       deliveredTo: [],
//       readBy: [],
//     };
...
//     // Broadcast once to everyone in room (including sender if you want)
//     io.to(roomId).emit("new_message", messageData);

//     // Map client tempId -> server message id once
//     if (message.tempId) {
//       socket.emit("message_delivered", {
//         tempId: message.tempId,
//         messageId: messageData.id,
...
//   // ====== DISCONNECT (applies to both Chat + Collab) ======
//   socket.on("disconnect", (reason) => {
//     console.log(`User disconnected: ${socket.id}, reason: ${reason}`);
...
//     // Remove from collaborative rooms
//     collaborativeRooms.forEach((room, roomId) => {
//       const userToRemove = Array.from(room.participants.values()).find(
//         (p) => p.socketId === socket.id
//       );
