...
import { useIdentity } from "../hooks/useIdentity";
// Wil create Live caht Stats - user/ connetxted --- 2.8
// import {} from "../services/chatService";

export default function ChatScreen() {
  // NEW: read optional params from navigation (lets parent pass currentUser / room)
  const route = useRoute<any>();
  const overrideUser = route.params?.currentUser; // { id, userName } if provided
  const initialRoomId = route.params?.roomId ?? "general";
  const initialRoomName = route.params?.roomName ?? "The OG Chat Space 💬";
  ...
  const participantsCount =
    rooms.find((r) => r.id === currentRoom)?.participants?.length ?? 1;

  // TEMP online flag until you surface real socket state from the hook
  // const online = !!isConnected; // <- when you expose it
  const online = true; // need to update w/ dynamic Stat -- 2.8
